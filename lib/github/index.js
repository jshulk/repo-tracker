...
GitHubRepo.prototype.commits = function(repos, callback){
    ...
    async.each( repos, function(repo, callback){
        self.github.repos.getCommits({ user: self.user, repo: repo}, function(error, response){
...
GitHubRepo.prototype.issues = function(repos, callback){
    var self = this,
        items = [];
    async.each(repos, function(repo, callback){
        self.github.issues.repoIssues({ user: self.user, repo: repo}, function(error, response){
           if( error ) return callback();
           if( response == null ) return callback();
